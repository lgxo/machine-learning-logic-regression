...
def posP(x_hat_mt, beta_v):
    index_v = x_hat_mt@beta_v       # \beta^T \hat x_i
    m = index_v.shape[1]
    result = np.ones((1, m))
    # 考虑到溢出，分情况处理
    for i in range(m):
        ...
        if element >= 0:
            # 此时 e^{-x} 不会溢出
            result[i] = 1/(1 + np.exp(-element))
        else:
            # 此时 e^x 不会溢出
            result[i] = np.exp(element)/(1 + np.exp(element))
    # check if every one has been handled
    if result[result == 2].size != 0:
        print("posP calculate error!")
        exit(0)
    return result
...
def newtonDescent(x_mt, y, penalty_newton=False, lambda_newton=0.):
    ...
    delta_max = np.max(beta_v + 1)
    loss = lossFuc(x_hat_mt, y, beta_v, penalty_newton, lambda_newton)
    while np.max(delta_max) > newton_accuracy:
